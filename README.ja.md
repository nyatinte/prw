...
各 [GitHub Release](https://github.com/nyatinte/prw/releases) にプラットフォーム別の実行ファイルが添付されています。
ダウンロードして実行権限を付与し、`$PATH` の通ったディレクトリに置くだけで使えます。
...
**Windows** — `prw-win-x64.exe` をダウンロードして `prw.exe` にリネームし、`%PATH%` の通ったフォルダに配置してください。
