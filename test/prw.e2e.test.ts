...
async function launchPrwSession({
  cwd,
  homeDir,
  args = [],
}: {
...
async function readTerminal(session: Session): Promise<string> {
...
async function closeSessionSafely(session: Session | undefined): Promise<void> {
  if (!session) {
    return;
  }
