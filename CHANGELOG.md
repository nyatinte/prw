...
- 3f38be5: Support XDG base directories for history storage.
  ...
  `prw` now stores history in `$XDG_STATE_HOME/prw/history.json` and falls back to `~/.local/state/prw/history.json` when `XDG_STATE_HOME` is unset. It no longer uses `~/.config/prw/history.json`.
